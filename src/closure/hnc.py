...
class HNC(object):
    ...
    def __init__(self, G, positive_seeds, negative_seeds,
                 arc_weight='weight', node_label='degree'):
        self._G = G
    ...
    def _add_node_weights(self):
        for node in self._G:
            ...
            self._G.nodes[node][self._node_label_parametric] = (
                - self._G.degree(node, weight=self._arc_weight)
                )

    def solve(self, value):
        # solve for single value with same lower/upper bound
        cuts, breakpoints = self._closure_problem.solve_parametric(value,
                                                                   value)
